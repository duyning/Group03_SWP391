...
public class CinemaRoomService {
    ...
    public boolean deleteRoom(int id) {
        // Kiểm tra xem phòng chiếu đã có lịch chiếu hoặc vé nào liên quan chưa
        if (roomRepository.hasDependencies(id)) {
            return false; // Không thể xóa vì đã có dữ liệu liên quan
        }
        roomRepository.delete(id);
        return true;
    }

    public boolean isRoomNameExists(String roomName, int cinemaId, Integer excludeRoomId) {
        CinemaRoom existingRoom = roomRepository.findByRoomNameAndCinemaId(roomName, cinemaId);
        if (existingRoom == null) {
            return false;
        }
        if (excludeRoomId != null && existingRoom.getId() == excludeRoomId) {
            return false;
        }
        return true;
    }
